...
void Calc::oper_mult(int &start, char expression[])	// multiplication
{
	...
	else
	{
		...
		{
			BigInt arg2 = buffer.top();
			buffer.pop();
			BigInt result = arg2 * arg1;
			buffer.push(result);
			for(int i = 0; i < 3; i++)
		    	expression[start] = ' ';
		}
...
void Calc::use_Variable(int & start, char expression[])
{
    ...
    n[i] = '\0';
    BigInt value = Var.search(n, isOK);
    if(!isOK)
    {
...
BigInt Calc::result(char expressionConv[])
{
